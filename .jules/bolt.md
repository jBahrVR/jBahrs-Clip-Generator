...
## 2025-05-20 - [Performance Anti-Pattern: Avoiding `np.abs` with min/max]
**Learning:** In NumPy arrays, avoiding `np.abs()` by calculating `np.max` and `np.min` separately (e.g., `(np.max(arr, axis=1) > threshold) | (np.min(arr, axis=1) < -threshold)`) to prevent intermediate memory allocations actually increases execution time by ~45%. The overhead of performing the min and max reductions separately outweighs the cost of the intermediate array allocation from `np.abs()`.
**Action:** Stick to `np.max(np.abs(arr))` for peak detection unless memory constraints strictly override execution speed.
